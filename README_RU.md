...
Полдюжины терминалов, несколько сессий ИИ-агентов — в таскбаре это одинаковые
иконки с одинаковыми подписями, и чтобы найти нужное окно, приходится перебирать
их все по Alt+Tab. Превью по наведению в Windows есть со времён Vista, но за
каждым надо охотиться отдельно.

montab — боковая панель-таскбар для Windows с **постоянными живыми превью** всех
открытых окон: взглянул на панель — и видно, в каком терминале ещё крутятся
тесты. Своя панель на каждом мониторе — и в ней только окна этого монитора.
Док слева или справа, резервирует рабочую область (развёрнутые окна не
перекрывают панель), обновление превью — в реальном времени через DWM-композитор,
практически бесплатно по ресурсам.

Один exe **~2,1 МБ** (.NET 11 NativeAOT, ноль зависимостей), ~5 МБ собственной
памяти с панелью на каждом мониторе, ~0% CPU в простое.

![montab: панель у правого края, живые превью сверху, свёрнутые окна полосками снизу](assets/screenshot-montab.png)

## Возможности

- Панель на каждом мониторе; в ленте — только окна этого монитора
  (перетащили окно на соседний экран — его тайл переехал следом).
  Любую панель можно выключить из трея.
- Живые превью, аспект сохраняется.
...
- Всплывающие уведомления (тосты, баннеры, OSD) таб не создают.
- Иконка в трее: левый клик убирает и возвращает все панели разом (иконка гаснет,
  настройки мониторов не теряются), правый — меню со списком мониторов.
- Настройки хранятся по мониторам (край, ширина, включена ли панель)
  в `%APPDATA%\montab\settings.json`.

## Управление

| Действие | Результат |
|---|---|
| Клик по превью неактивного окна | Переключение в окно (задержка ~150 мс — отличение от двойного клика) |
| Клик по превью активного окна | Увести окно под все остальные, не сворачивая; активным становится то, что было под ним (единственное окно монитора остаётся как есть) |
...
| Драг по оверлей-скроллбару | Прокрутка для трекпадов; широкая полупрозрачная полоса у внешнего края появляется при наведении, когда лента переполнена. Крестики ✕ кликабельны сквозь неё |
...
| Перетаскивание за «ручку» сверху или пустую зону | Смена края на своём мониторе; бросок на монитор без панели переносит её туда |
| Перетаскивание внутреннего края | Ширина панели (3–50% ширины монитора) |
| Правый клик по превью | Свернуть окно |
| Правый клик по «ручке» сверху или пустой зоне | Меню: край дока, скрыть панель на этом мониторе, автозапуск, выход |
| Левый клик по иконке в трее | Скрыть/вернуть панели на всех мониторах (иконка гаснет; настройки сохраняются) |
| Правый клик по иконке в трее | Меню: по монитору — «включено» и «слева/справа»; автозапуск, выход |
